...
### Added
- **`LOGIN_CHECK_SYSTEM_USER`** (default `false`) — when enabled, the login form is verified against the Pi's real system account over SSH rather than against `LOGIN_PASS`, so the password is checked by the OS and cannot drift out of sync with the system one. `LOGIN_PASS` is then unused (an empty value still counts as configured, and the default-password warning is suppressed). Only `SSH_USER` may sign in: accepting any system account would let a low-privileged user have GumCP run commands as `SSH_USER`. Left off by default, so the login stays an ordinary config credential unless you opt in.
